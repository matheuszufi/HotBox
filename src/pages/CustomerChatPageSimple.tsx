...
const CustomerChatPageSimple: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeChat = async () => {
      try {
        ...
        const connectionTest = await simpleChatService.testConnection();
        if (!connectionTest.success) {
          setError(connectionTest.error || 'Erro de conectividade');
          setLoading(false);
          return;
        }

        // Buscar ou criar chat
        const chatId = await simpleChatService.getOrCreateSimpleChat({
          id: user.uid,
        ...
        // Buscar mensagens
        const chatMessages = await simpleChatService.getChatMessages(chatId);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Enviar mensagem
      await simpleChatService.sendSimpleMessage(chatId, {
        senderId: user.uid,
      ...
      const updatedMessages = await simpleChatService.getChatMessages(chatId);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-sm text-gray-500">Verificando conectividade...</p>
        </div>
      </div>
    );
  }
...
export default CustomerChatPageSimple;
